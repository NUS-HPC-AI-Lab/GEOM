...
class MetaGtt:
    ...
    def synset_save(self):
        ...
        if args.soft_label == True:
            feat_syn_eval, label_syn_eval,tem_eval = copy.deepcopy(feat_save.detach()), copy.deepcopy(
                eval_labs.detach()), copy.deepcopy(tem.detach())  # avoid any unaware modification
        else:
            feat_syn_eval, label_syn_eval,tem_eval = copy.deepcopy(feat_save.detach()), copy.deepcopy(
                eval_labs.detach()), tem  # avoid any unaware modification

        adj_syn_eval = torch.eye(feat_syn_eval.shape[0]).to(self.device)


        return feat_syn_eval, adj_syn_eval, label_syn_eval,tem_eval
    ...
    def test(self, args, data, device, model_type, nruns):
        ...
        if args.whole_data != 1:
            feat_syn, adj_syn, labels_syn,tem = self.synset_save()
        ...
        for i in range(train_iters):
            ...
            if args.whole_data == 1:
                ...
                # loss_train = torch.nn.KLDivLoss(reduction="batchmean", log_target=True)(output_syn/tem, labels_syn/tem)
                if args.soft_label == True:
...
#-------------------------------------softlabel-------------------------------------------------------end-----------------------------------------------------------------#
        ...
        # best_accs_eval = {m: 0 for m in model_eval_pool}
        # best_accs_eval_iter = {m: 0 for m in model_eval_pool}
        ...
        for it in range(0, args.ITER + 1):
            ...
            # feat_syn = self.feat_syn
            # adj_syn_norm = utils.normalize_adj_tensor(self.adj_syn_init, sparse=True) 'flickr',
            
            # adj_syn_input = adj_syn_norm


            for step in range(args.syn_steps):  # 用synset训练model
                ...
                if args.soft_label == True:
                    loss_syn = torch.nn.KLDivLoss(reduction="batchmean", log_target=True)(output_syn/self.tem, self.labels_syn/self.tem)
                    acc_syn = utils.accuracy(output_syn, torch.argmax(self.labels_syn, dim=1))
                else:
                    loss_syn = F.nll_loss(output_syn, self.labels_syn)
                    acc_syn = utils.accuracy(output_syn, self.labels_syn)
                # loss_syn = self.SoftCrossEntropy(output_syn, self.labels_syn)
            ...
            # cosine_dis = 1 - torch.nn.functional.cosine_similarity(student_params[-1], target_params, dim=0)
            # cosine_list = 1 - torch.nn.functional.cosine_similarity(starting_params, target_params, dim=0)
            
            param_loss += torch.norm(student_params[-1] - target_params, 2)
            param_dist += torch.norm(starting_params - target_params, 2)
            # param_dist += cosine_list
            
            # print(starting_params.shape, target_params.shape)
            ...
            if args.soft_label == True:
                loss_clom = torch.nn.KLDivLoss(reduction="batchmean", log_target=True)(output_clom/self.tem, self.labels_syn/self.tem)
            else:
                loss_clom = F.nll_loss(output_clom, self.labels_syn)
            ...
            # loss_clom = self.SoftCrossEntropy(output_clom, self.labels_syn)
            
            # print('loss_clom:{}', loss_clom)
            
            # logging.info('grand_loss = {:.4f}, loss_clom = {:.4f}'.format(grand_loss.item(), loss_clom.item()))
            ...
            if it in eval_it_pool and it > 0:
                for model_eval in model_eval_pool:
                    ...
                    feat_syn_save, adj_syn_save, label_syn_save,tem_save = self.synset_save()
                    ...
                    if best_acc_test > best_accs_test[model_eval]:
                        ...
                        if args.soft_label == True:
                            torch.save(tem_save,
                                    f'{args.log_dir}/tem_{args.dataset}_{args.reduction_rate}_best_eval_acc_{args.seed_student}_ours.pt')
            ...
            if it % 1000 == 0 or it==args.ITER:
                ...
                if args.soft_label == True:
                    torch.save(tem_save,
                                    f'{args.log_dir}/tem_{args.dataset}_{args.reduction_rate}_{it}_{args.seed_student}_ours.pt')
