...
def test(args, data, device, model_type, nruns):
    ...
    for i in range(train_iters):
        ...
        if args.whole_data == 1:
            ...
            optimizer.zero_grad()
            _,output_syn = model.forward(feat_syn, adj_syn)
            # loss_train = SoftCrossEntropy(output_syn, labels_syn)
            try:
                loss_train = torch.nn.KLDivLoss(reduction="batchmean", log_target=True)(output_syn, labels_syn)
            # acc_syn = utils.accuracy_soft(output_syn, labels_syn)
        ...
        if i % 1 == 0:
            ...
            if args.dataset in ['reddit', 'flickr']:
                ...
                logging.info(
                    "Test full set results : loss= {:.4f}, accuracy= {:.4f}".format(
                        loss_test.item(),
                        acc_test.item()))
...
def get_syn_data(args, data, device, model_type=None):
    if args.best_ntk_score==1:
        # feat_syn = torch.load(f'{args.load_path}/feat_{args.dataset}_{args.reduction_rate}_best_ntk_score_{args.tr_seed}_ours.pt',
        #                       map_location='cpu')
        feat_syn = torch.load(f'{args.load_path}/feat_{args.dataset}_{args.reduction_rate}_best_eval_acc_{args.tr_seed}_ours.pt',
                              map_location='cpu')
        labels_syn = torch.load(
            f'{args.load_path}/label_{args.dataset}_{args.reduction_rate}_best_eval_acc_{args.tr_seed}_ours.pt',
            map_location='cpu')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--eval_way", type=str, default='acc', help="ntk eval")


    parser.add_argument('--max_start_epoch', type=int, default=300)      #1000 #500  
    parser.add_argument('--min_start_epoch', type=int, default=0)     #400 #100
    ...
    #                                                                              "%Y%m%d-%H%M%S-%f"))
    log_dir = './logs/Test/val-kd2-{}-model_{}-reduce_{}-test_wd-{}-test_model_iters-{}-test_dropout{}-test_opt_type{}'.format(args.dataset, 
    args.test_model_type, str(args.reduction_rate),args.test_wd,args.test_model_iters,args.test_dropout,args.test_opt_type)
