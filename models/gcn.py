...
class GCN(nn.Module):
    ...
    def forward(self, x, adj):
        for ix, layer in enumerate(self.layers):  # layer = 2 
            x = layer(x, adj)                     # last: class   last-1:归一化
    ...
    @torch.no_grad()
    def predict(self, features=None, adj=None): # 计算时不影响函数梯度
        ...
        self.eval()
        if features is None and adj is None: # 判断有没有输入
